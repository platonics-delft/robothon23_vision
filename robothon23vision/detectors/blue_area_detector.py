...
from robothon23.detectors.errors import NoAreaDetectedError


@dataclass
class BlueAreaDetectorConfig:
    kernel: bool = False
    min_area: int = 300
    h_low: int = 100
    s_low: int = 240
...
class BlueAreaDetector(FeatureDetector):
    def __init__(self, img_in, **config):
        super().__init__(img_in, 'blue_area')
        self._config = BlueAreaDetectorConfig(**config)
    ...
    def detect(self) -> dict:
        ...
        if len(centers) == 0:
            raise NoAreaDetectedError("No blue area found.")
